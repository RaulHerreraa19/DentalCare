async function test() {
  const baseURL = "https://dentalcare-irma.onrender.com/api/v1";
  ...
  try {
    ...
    if (res.status === 200) {
      console.log(
        "3. Receptionist collected payment. Result:",
    ...
    const cashEntry = res.data.data.find(
      (c) => c.appointment_id === appointmentId,
    );
    console.log(
      "4. CashRegister entry amount:",
      cashEntry ? cashEntry.amount : "Not found",
    );
  } catch (err) {
    console.error(err);
  }
