    ...
    const fakeDb = {
      appointment: {
        findFirst: async ({ where, include }) => {
          if (where.id === 'apt-1' && where.organization_id === 'org-1') return fakeAppointment;
          return null;
        }
      }
    };

    const AppointmentsService = proxyquire('../../src/modules/appointments/appointments.service', {
      '../../config/database': fakeDb,
      '../../utils/AppError': class AppError extends Error { constructor(msg, code){ super(msg); this.statusCode = code; } }
    });
